...
export const guestGamePersistence: GamePersistence = {
  ...
  async update(id, changes, expectedRevision) {
    ...
    const saved = gameDetailSchema.parse({
      ...
      userMainAttacker: editableChanges.userMainAttacker?.trim() || recalculated.userMainAttacker?.trim() || games[index].userMainAttacker,
      opponentMainAttacker: editableChanges.opponentMainAttacker?.trim() || recalculated.opponentMainAttacker?.trim() || games[index].opponentMainAttacker,
