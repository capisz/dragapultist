  ...
  const renderArchetypeControls = (side: Side, label: string) => {
    ...
    return (
      <div className="space-y-2">
        ...
        <ArchetypeSelector value={value} label={label} emptyValue={UNKNOWN_ARCHETYPE} emptyLabel="Not set / Unknown" onValueChange={(nextValue) => {
          setArchetypeValueForSide(side, nextValue)
          if (!isCustomArchetypeId(nextValue)) closeCustomBuilder(side)
        }} />
